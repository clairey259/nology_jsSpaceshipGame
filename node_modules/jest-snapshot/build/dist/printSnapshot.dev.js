...
var getSnapshotColorForChalkInstance = function getSnapshotColorForChalkInstance(chalkInstance) {
  var level = chalkInstance.level;
...
var getReceivedColorForChalkInstance = function getReceivedColorForChalkInstance(chalkInstance) {
  var level = chalkInstance.level;
...
var aSnapshotColor = getSnapshotColorForChalkInstance(_chalk["default"]);
exports.aSnapshotColor = aSnapshotColor;
var bReceivedColor = getReceivedColorForChalkInstance(_chalk["default"]);
...
var SNAPSHOT_ARG = 'snapshot';
exports.SNAPSHOT_ARG = SNAPSHOT_ARG;
var PROPERTIES_ARG = 'properties';
exports.PROPERTIES_ARG = PROPERTIES_ARG;

var matcherHintFromConfig = function matcherHintFromConfig(_ref, isUpdatable) {
  var _ref$context = _ref.context,
      isNot = _ref$context.isNot,
      promise = _ref$context.promise,
      hint = _ref.hint,
      inlineSnapshot = _ref.inlineSnapshot,
      matcherName = _ref.matcherName,
...
var joinDiffs = function joinDiffs(diffs, op, hasCommon) {
  return diffs.reduce(function (reduced, diff) {
    return reduced + (diff[0] === _jestDiff.DIFF_EQUAL ? diff[1] : diff[0] !== op ? '' : hasCommon ? (0, _jestMatcherUtils.INVERTED_COLOR)(diff[1]) : diff[1]);
  }, '');
};

var isLineDiffable = function isLineDiffable(received) {
...
var printExpected = function printExpected(val) {
  return (0, _jestMatcherUtils.EXPECTED_COLOR)((0, _utils.minify)(val));
};

exports.printExpected = printExpected;

var printReceived = function printReceived(val) {
  return (0, _jestMatcherUtils.RECEIVED_COLOR)((0, _utils.minify)(val));
};

exports.printReceived = printReceived;

var printPropertiesAndReceived = function printPropertiesAndReceived(properties, received, expand // CLI options: true if `--expand` or false if `--no-expand`
) {
  var aAnnotation = 'Expected properties';
  var bAnnotation = 'Received value';

  if (isLineDiffable(properties) && isLineDiffable(received)) {
    return (0, _jestDiff.diffLinesUnified)((0, _utils.serialize)(properties).split('\n'), (0, _utils.serialize)((0, _expectUtils.getObjectSubset)(received, properties)).split('\n'), {
  ...
  var printLabel = (0, _jestMatcherUtils.getLabelPrinter)(aAnnotation, bAnnotation);
  return "".concat(printLabel(aAnnotation) + printExpected(properties), "\n").concat(printLabel(bAnnotation)).concat(printReceived(received));
};

exports.printPropertiesAndReceived = printPropertiesAndReceived;
var MAX_DIFF_STRING_LENGTH = 20000;

var printSnapshotAndReceived = function printSnapshotAndReceived(a, b, received, expand, snapshotFormat) {
  var aAnnotation = 'Snapshot';
  var bAnnotation = 'Received';
  var aColor = aSnapshotColor;
  var bColor = bReceivedColor;
  var options = {
    aAnnotation: aAnnotation,
    aColor: aColor,
    bAnnotation: bAnnotation,
    ...
    if (a.length >= 2 && a.startsWith('"') && a.endsWith('"') && b === (0, _prettyFormat.format)(received)) {
      ...
      if (!a.includes('\n') && !b.includes('\n')) {
        ...
        if (a.length - 2 <= MAX_DIFF_STRING_LENGTH && b.length - 2 <= MAX_DIFF_STRING_LENGTH) {
          var diffs = (0, _jestDiff.diffStringsRaw)(a.slice(1, -1), b.slice(1, -1), true);
          var hasCommon = diffs.some(function (diff) {
            return diff[0] === _jestDiff.DIFF_EQUAL;
          });
          aQuoted = "\"".concat(joinDiffs(diffs, _jestDiff.DIFF_DELETE, hasCommon), "\"");
          bQuoted = "\"".concat(joinDiffs(diffs, _jestDiff.DIFF_INSERT, hasCommon), "\"");
        }

        var _printLabel = (0, _jestMatcherUtils.getLabelPrinter)(aAnnotation, bAnnotation);

        return "".concat(_printLabel(aAnnotation) + aColor(aQuoted), "\n").concat(_printLabel(bAnnotation)).concat(bColor(bQuoted));
  ...
  if (isLineDiffable(received)) {
    ...
    var b0 = (0, _utils.serialize)(received, 0, snapshotFormat);

    if (b0 !== b) {
      var aLines0 = (0, _dedentLines.dedentLines)(aLines2);

      if (aLines0 !== null) {
        // Compare lines without indentation.
        var bLines0 = b0.split('\n');
  ...
  var printLabel = (0, _jestMatcherUtils.getLabelPrinter)(aAnnotation, bAnnotation);
  return "".concat(printLabel(aAnnotation) + aColor(a), "\n").concat(printLabel(bAnnotation)).concat(bColor(b));
};
