...
var jestWriteFile = globalThis[_Symbol["for"]('jest-native-write-file')] || fs.writeFileSync;

var _Symbol = globalThis['jest-symbol-do-not-touch'] || globalThis.Symbol;

var jestReadFile = globalThis[_Symbol["for"]('jest-native-read-file')] || fs.readFileSync;
...
var validateSnapshotVersion = function validateSnapshotVersion(snapshotContents) {
  ...
  if (version < SNAPSHOT_VERSION) {
    ...
    _chalk["default"].red("".concat(_chalk["default"].red.bold('Outdated snapshot'), ": The version of the snapshot ") + 'file associated with this test is outdated. The snapshot file ' + 'version ensures that all developers on a project are using ' + 'the same version of Jest. ' + 'Please update all snapshots during this upgrade of Jest.') + '\n\n' + "Expected: v".concat(SNAPSHOT_VERSION, "\n") + "Received: v".concat(version, "\n\n") + SNAPSHOT_VERSION_WARNING);
  }

  if (version > SNAPSHOT_VERSION) {
    return new Error( // eslint-disable-next-line prefer-template
    _chalk["default"].red("".concat(_chalk["default"].red.bold('Outdated Jest version'), ": The version of this ") + 'snapshot file indicates that this project is meant to be used ' + 'with a newer version of Jest. The snapshot file version ensures ' + 'that all developers on a project are using the same version of ' + 'Jest. Please update your version of Jest and re-run the tests.') + '\n\n' + "Expected: v".concat(SNAPSHOT_VERSION, "\n") + "Received: v".concat(version));
  }
...
var serialize = function serialize(val) {
  ...
  return normalizeNewlines((0, _prettyFormat.format)(val, _objectSpread({
...
var minify = function minify(val) {
  return (0, _prettyFormat.format)(val, {
    escapeRegex: escapeRegex,
    min: true,
    plugins: (0, _plugins.getSerializers)(),
    printFunctionName: printFunctionName
...
var saveSnapshotFile = function saveSnapshotFile(snapshotData, snapshotPath) {
  ...
  jestWriteFile(snapshotPath, "".concat(writeSnapshotVersion(), "\n\n").concat(snapshots.join('\n\n'), "\n"));
};
