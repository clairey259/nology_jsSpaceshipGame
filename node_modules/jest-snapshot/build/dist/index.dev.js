...
var jestExistsFile = globalThis[_Symbol["for"]('jest-native-exists-file')] || fs.existsSync;
...
var printSnapshotName = function printSnapshotName() {
  ...
  return "Snapshot name: `".concat(hasNames ? (0, _utils.escapeBacktickString)(concatenatedBlockNames) : '').concat(hasNames && hasHint ? ': ' : '').concat(hasHint ? (0, _jestMatcherUtils.BOLD_WEIGHT)((0, _utils.escapeBacktickString)(hint)) : '', " ").concat(count, "`");
};
...
var cleanup = function cleanup(hasteFS, update, snapshotResolver, testPathIgnorePatterns) {
  var pattern = "\\.".concat(_SnapshotResolver.EXTENSION, "$");
...
var toMatchSnapshot = function toMatchSnapshot(received, propertiesOrHint, hint) {
  ...
  if (length === 2 && typeof propertiesOrHint === 'string') {
    ...
    if (Array.isArray(propertiesOrHint) || _typeof(propertiesOrHint) !== 'object' || propertiesOrHint === null) {
      var options = {
        isNot: this.isNot,
...
var toMatchInlineSnapshot = function toMatchInlineSnapshot(received, propertiesOrSnapshot, inlineSnapshot) {
  ...
  if (length === 2 && typeof propertiesOrSnapshot === 'string') {
    ...
    if (Array.isArray(propertiesOrSnapshot) || _typeof(propertiesOrSnapshot) !== 'object' || propertiesOrSnapshot === null) {
...
var _toMatchSnapshot = function _toMatchSnapshot(config) {
  ...
  if (snapshotState == null) {
    ...
    throw new Error("".concat((0, _printSnapshot.matcherHintFromConfig)(config, false), "\n\n") + 'Snapshot state must be initialized' + "\n\n".concat((0, _jestMatcherUtils.printWithType)('Snapshot state', snapshotState, _jestMatcherUtils.stringify)));
  }
  ...
  if (_typeof(properties) === 'object') {
    ...
    if (!propertyPass) {
      ...
      var _message = function _message() {
        return "".concat((0, _printSnapshot.matcherHintFromConfig)(config, false), "\n\n").concat(printSnapshotName(currentTestName, hint, _count), "\n\n").concat((0, _printSnapshot.printPropertiesAndReceived)(properties, received, snapshotState.expand));
      };
  ...
  var message = expected === undefined ? function () {
    return "".concat((0, _printSnapshot.matcherHintFromConfig)(config, true), "\n\n").concat(printSnapshotName(currentTestName, hint, count), "\n\n") + "New snapshot was ".concat((0, _jestMatcherUtils.BOLD_WEIGHT)('not written'), ". The update flag ") + 'must be explicitly passed to write a new snapshot.\n\n' + 'This is likely because this test is run in a continuous integration ' + '(CI) environment in which snapshots are not written by default.\n\n' + "Received:".concat(actual.includes('\n') ? '\n' : ' ').concat((0, _printSnapshot.bReceivedColor)(actual));
  } : function () {
    return "".concat((0, _printSnapshot.matcherHintFromConfig)(config, true), "\n\n").concat(printSnapshotName(currentTestName, hint, count), "\n\n").concat((0, _printSnapshot.printSnapshotAndReceived)(expected, actual, received, snapshotState.expand, snapshotState.snapshotFormat));
...
var toThrowErrorMatchingSnapshot = function toThrowErrorMatchingSnapshot(received, hint, fromPromise) {
