...
function printNodeFields(key, readme) {
  if (Object.keys(t.NODE_FIELDS[key]).length > 0) {
    ...
    Object.keys(t.NODE_FIELDS[key]).sort(function (fieldA, fieldB) {
      var indexA = t.BUILDER_KEYS[key].indexOf(fieldA);
      var indexB = t.BUILDER_KEYS[key].indexOf(fieldB);
      if (indexA === indexB) return fieldA < fieldB ? -1 : 1;
      if (indexA === -1) return 1;
      if (indexB === -1) return -1;
      return indexA - indexB;
    }).forEach(function (field) {
      var defaultValue = t.NODE_FIELDS[key][field]["default"];
      var fieldDescription = ["`" + field + "`"];
      var validator = t.NODE_FIELDS[key][field].validate;
...
Object.keys(t.BUILDER_KEYS).sort().forEach(function (key) {
  ...
  readme.push("t." + (0, _toFunctionName["default"])(key) + "(" + t.BUILDER_KEYS[key].join(", ") + ");");
  readme.push("```");
...
function generateMapAliasToNodeTypes() {
  ...
  for (var _i = 0, _Object$keys = Object.keys(t.ALIAS_KEYS); _i < _Object$keys.length; _i++) {
    var nodeType = _Object$keys[_i];
    var aliases = t.ALIAS_KEYS[nodeType];
    if (!aliases) continue;
