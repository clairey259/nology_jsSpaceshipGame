...
var t = _interopRequireWildcard(require("../../lib/index.js"));
...
var _loop = function _loop(type) {
  ...
  if (type !== "Super" && type !== "Import") {
    lines.push("export function ".concat((0, _toFunctionName["default"])(type), "(").concat(args.join(", "), "): ").concat(type, ";"));
  } else {
...
try {
  for (var _iterator = t.TYPES[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
    ...
    var isDeprecated = !!t.DEPRECATED_KEYS[typeName];
    var realName = isDeprecated ? t.DEPRECATED_KEYS[typeName] : typeName;
    var result = t.NODE_FIELDS[realName] || t.FLIPPED_ALIAS_KEYS[realName] ? "node is ".concat(realName) : "boolean";

    if (isDeprecated) {
      lines.push("/** @deprecated Use `is".concat(realName, "` */"));
    }

    lines.push("export function is".concat(typeName, "(node: object | null | undefined, opts?: object | null): ").concat(result, ";"));

    if (isDeprecated) {
      lines.push("/** @deprecated Use `assert".concat(realName, "` */"));
    }

    lines.push("export function assert".concat(typeName, "(node: object | null | undefined, opts?: object | null): void;"));
  }
} catch (err) {
