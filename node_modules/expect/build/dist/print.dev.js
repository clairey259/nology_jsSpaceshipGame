...
var printReceivedStringContainExpectedSubstring = function printReceivedStringContainExpectedSubstring(received, start, length // not end
) {
  return (0, _jestMatcherUtils.RECEIVED_COLOR)("\"".concat(printSubstring(received.slice(0, start))).concat((0, _jestMatcherUtils.INVERTED_COLOR)(printSubstring(received.slice(start, start + length)))).concat(printSubstring(received.slice(start + length)), "\""));
};
...
var printReceivedArrayContainExpectedItem = function printReceivedArrayContainExpectedItem(received, index) {
  return (0, _jestMatcherUtils.RECEIVED_COLOR)("[".concat(received.map(function (item, i) {
...
var printExpectedConstructorName = function printExpectedConstructorName(label, expected) {
  return "".concat(printConstructorName(label, expected, false, true), "\n");
};
...
var printExpectedConstructorNameNot = function printExpectedConstructorNameNot(label, expected) {
  return "".concat(printConstructorName(label, expected, true, true), "\n");
};
...
var printReceivedConstructorName = function printReceivedConstructorName(label, received) {
  return "".concat(printConstructorName(label, received, false, false), "\n");
...
var printReceivedConstructorNameNot = function printReceivedConstructorNameNot(label, received, expected) {
  return typeof expected.name === 'string' && expected.name.length !== 0 && typeof received.name === 'string' && received.name.length !== 0 ? "".concat(printConstructorName(label, received, true, false), " ").concat(Object.getPrototypeOf(received) === expected ? 'extends' : 'extends … extends', " ").concat((0, _jestMatcherUtils.EXPECTED_COLOR)(expected.name), "\n") : "".concat(printConstructorName(label, received, false, false), "\n");
};
