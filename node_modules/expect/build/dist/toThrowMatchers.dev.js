...
var toThrowExpectedRegExp = function toThrowExpectedRegExp(matcherName, options, thrown, expected) {
  ...
  var message = pass ? function () {
    ...
    return (// eslint-disable-next-line prefer-template
      (0, _jestMatcherUtils.matcherHint)(matcherName, undefined, undefined, options) + '\n\n' + formatExpected('Expected pattern: ', expected) + (thrown === null ? "\n".concat(DID_NOT_THROW) : thrown.hasMessage ? formatReceived('Received message: ', thrown, 'message') + formatStack(thrown) : formatReceived('Received value:   ', thrown, 'value'))
    );
  };
  return {
    message: message,
...
var toThrowExpectedObject = function toThrowExpectedObject(matcherName, options, thrown, expected) {
  ...
  var message = pass ? function () {
    ...
    return (// eslint-disable-next-line prefer-template
      ...
      ? formatExpected('Expected message: ', expected.message) + '\n' + DID_NOT_THROW : thrown.hasMessage // eslint-disable-next-line prefer-template
      ? (0, _jestMatcherUtils.printDiffOrStringify)(expected.message, thrown.message, 'Expected message', 'Received message', true) + '\n' + formatStack(thrown) : formatExpected('Expected message: ', expected.message) + formatReceived('Received value:   ', thrown, 'value'))
    );
  };
  return {
    message: message,
...
var toThrowExpectedClass = function toThrowExpectedClass(matcherName, options, thrown, expected) {
  ...
  var message = pass ? function () {
    ...
    return (// eslint-disable-next-line prefer-template
      (0, _jestMatcherUtils.matcherHint)(matcherName, undefined, undefined, options) + '\n\n' + (0, _print.printExpectedConstructorName)('Expected constructor', expected) + (thrown === null ? "\n".concat(DID_NOT_THROW) : "".concat(thrown.value != null && typeof thrown.value.constructor === 'function' ? (0, _print.printReceivedConstructorName)('Received constructor', thrown.value.constructor) : '', "\n").concat(thrown.hasMessage ? formatReceived('Received message: ', thrown, 'message') + formatStack(thrown) : formatReceived('Received value: ', thrown, 'value')))
    );
  };
  return {
    message: message,
...
var formatExpected = function formatExpected(label, expected) {
  return "".concat(label + (0, _jestMatcherUtils.printExpected)(expected), "\n");
};
...
var formatReceived = function formatReceived(label, thrown, key, expected) {
  ...
  if (key === 'message') {
    ...
    if (typeof expected === 'string') {
      ...
      if (index !== -1) {
        return "".concat(label + (0, _print.printReceivedStringContainExpectedSubstring)(message, index, expected.length), "\n");
      }
    } else if (expected instanceof RegExp) {
      return "".concat(label + (0, _print.printReceivedStringContainExpectedResult)(message, typeof expected.exec === 'function' ? expected.exec(message) : null), "\n");
    }

    return "".concat(label + (0, _jestMatcherUtils.printReceived)(message), "\n");
  }

  if (key === 'name') {
    return thrown.isError ? "".concat(label + (0, _jestMatcherUtils.printReceived)(thrown.value.name), "\n") : '';
  }

  if (key === 'value') {
    return thrown.isError ? '' : "".concat(label + (0, _jestMatcherUtils.printReceived)(thrown.value), "\n");
  }
