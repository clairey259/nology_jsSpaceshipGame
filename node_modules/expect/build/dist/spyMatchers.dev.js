...
var isEqualValue = function isEqualValue(expected, received) {
  return (0, _expectUtils.equals)(expected, received, [_expectUtils.iterableEquality]);
};

var isEqualCall = function isEqualCall(expected, received) {
  return received.length === expected.length && isEqualValue(expected, received);
};
...
var printNumberOfReturns = function printNumberOfReturns(countReturns, countCalls) {
  return "\nNumber of returns: ".concat((0, _jestMatcherUtils.printReceived)(countReturns)).concat(countCalls !== countReturns ? "\nNumber of calls:   ".concat((0, _jestMatcherUtils.printReceived)(countCalls)) : '');
...
var getRightAlignedPrinter = function getRightAlignedPrinter(label) {
  ...
  return function (string, isExpectedCall) {
    return (isExpectedCall ? "->".concat(' '.repeat(Math.max(0, index - 2 - string.length))) : ' '.repeat(Math.max(index - string.length))) + string + suffix;
  };
};
...
var printReceivedCallsNegative = function printReceivedCallsNegative(expected, indexedCalls, isOnlyCall, iExpectedCall) {
  ...
  if (isOnlyCall) {
    return "".concat(label + printReceivedArgs(indexedCalls[0], expected), "\n");
  }

  var printAligned = getRightAlignedPrinter(label);
  return "Received\n".concat(indexedCalls.reduce(function (printed, _ref) {
    var _ref2 = _slicedToArray(_ref, 2),
        i = _ref2[0],
        args = _ref2[1];

    return "".concat(printed + printAligned(String(i + 1), i === iExpectedCall) + printReceivedArgs(args, expected), "\n");
  }, ''));
};
...
var printExpectedReceivedCallsPositive = function printExpectedReceivedCallsPositive(expected, indexedCalls, expand, isOnlyCall, iExpectedCall) {
  ...
  if (isOnlyCall && (iExpectedCall === 0 || iExpectedCall === undefined)) {
    ...
    if (isLineDiffableCall(expected, received)) {
      ...
      for (var i = 0; i < length; i += 1) {
        if (i < expected.length && i < received.length) {
          ...
          if (isLineDiffableArg(expected[i], received[i])) {
            ...
            if (typeof difference === 'string' && difference.includes('- Expected') && difference.includes('+ Received')) {
              ...
              lines.push("".concat(difference.split('\n').slice(3).join('\n'), ","));
              continue;
            }
          }
        }
        ...
        if (i < expected.length) {
          lines.push("".concat((0, _jestMatcherUtils.EXPECTED_COLOR)("- ".concat((0, _jestMatcherUtils.stringify)(expected[i]))), ","));
        }

        if (i < received.length) {
          lines.push("".concat((0, _jestMatcherUtils.RECEIVED_COLOR)("+ ".concat((0, _jestMatcherUtils.stringify)(received[i]))), ","));
        }
      }
    ...
    return "".concat(expectedLine + label + printReceivedArgs(received, expected), "\n");
  }
  ...
  return (// eslint-disable-next-line prefer-template
    expectedLine + 'Received\n' + indexedCalls.reduce(function (printed, _ref3) {
      ...
      return "".concat(printed + ((i === iExpectedCall || iExpectedCall === undefined) && isLineDiffableCall(expected, received) ? aligned.replace(': ', '\n') + printDiffCall(expected, received, expand) : aligned + printReceivedArgs(received, expected)), "\n");
    }, '')
  );
};
  ...
  return received.map(function (arg, i) {
    if (i < expected.length) {
      if (isEqualValue(expected[i], arg)) {
        return "".concat(indentation, "  ").concat(printCommon(arg), ",");
      }
      ...
      if (isLineDiffableArg(expected[i], arg)) {
        ...
        if (typeof difference === 'string' && difference.includes('- Expected') && difference.includes('+ Received')) {
          ...
          // Omit annotation in case multiple args have diff.
          return "".concat(difference.split('\n').slice(3).map(function (line) {
    ...
    return "".concat(indentation + (i < expected.length ? "  ".concat((0, _jestMatcherUtils.printReceived)(arg)) : (0, _jestMatcherUtils.RECEIVED_COLOR)("+ ".concat((0, _jestMatcherUtils.stringify)(arg)))), ",");
  }).join('\n');
};
...
var printReceivedResults = function printReceivedResults(label, expected, indexedResults, isOnlyCall, iExpectedCall) {
  ...
  if (isOnlyCall && (iExpectedCall === 0 || iExpectedCall === undefined)) {
    return "".concat(label + printResult(indexedResults[0][1], expected), "\n");
  }
  ...
  return (// eslint-disable-next-line prefer-template
    label.replace(':', '').trim() + '\n' + indexedResults.reduce(function (printed, _ref5) {
      ...
      return "".concat(printed + printAligned(String(i + 1), i === iExpectedCall) + printResult(result, expected), "\n");
    }, '')
  );
};
