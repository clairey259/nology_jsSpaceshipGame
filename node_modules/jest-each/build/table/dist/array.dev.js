...
var ESCAPED_PLACEHOLDER_PREFIX = /%%/g;
...
function array(title, arrayTable) {
  if (isTemplates(title, arrayTable)) {
    return arrayTable.map(function (template, index) {
      return {
        arguments: [template],
...
var isTemplates = function isTemplates(title, arrayTable) {
  return !SUPPORTED_PLACEHOLDERS.test(interpolateEscapedPlaceholders(title)) && !isTable(arrayTable) && arrayTable.every(function (col) {
    return col != null && _typeof(col) === 'object';
  });
};
...
var normalisePlaceholderValue = function normalisePlaceholderValue(value) {
  return typeof value === 'string' ? value.replace(new RegExp(PLACEHOLDER_PREFIX, 'g'), JEST_EACH_PLACEHOLDER_ESCAPE) : value;
};
...
var interpolateEscapedPlaceholders = function interpolateEscapedPlaceholders(title) {
  return title.replace(ESCAPED_PLACEHOLDER_PREFIX, JEST_EACH_PLACEHOLDER_ESCAPE);
};
...
var interpolatePrettyPlaceholder = function interpolatePrettyPlaceholder(title, value) {
  return title.replace(PRETTY_PLACEHOLDER, (0, _prettyFormat().format)(value, {
    maxDepth: 1,
    min: true
  }));
};