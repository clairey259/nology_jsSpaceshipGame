...
function check(argv) {
  ...
  for (var _i = 0, _arr = ['onlyChanged', 'lastCommit', 'changedFilesWithAncestor', 'changedSince']; _i < _arr.length; _i++) {
    var key = _arr[_i];
  ...
  if (argv.config && !(0, _jestConfig().isJSONString)(argv.config) && !argv.config.match(new RegExp("\\.(".concat(_jestConfig().constants.JEST_CONFIG_EXT_ORDER.map(function (e) {
    return e.substring(1);
  }).join('|'), ")$"), 'i'))) {
    throw new Error("The --config option requires a JSON string literal, or a file path with one of these extensions: ".concat(_jestConfig().constants.JEST_CONFIG_EXT_ORDER.join(', '), ".\nExample usage: jest --config ./jest.config.js"));
  }

  return true;
}

var usage = 'Usage: $0 [--config=<pathToConfigFile>] [TestPathPattern]';
exports.usage = usage;
var docs = 'Documentation: https://jestjs.io/'; // The default values are all set in jest-config
