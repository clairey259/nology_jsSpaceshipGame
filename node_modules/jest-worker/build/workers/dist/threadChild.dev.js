...
var messageListener = function messageListener(request) {
  switch (request[0]) {
    ...
    default:
      throw new TypeError("Unexpected request from parent process: ".concat(request[0]));
  }
};
...
function reportMemoryUsage() {
  ...
  var msg = [_types.PARENT_MESSAGE_MEM_USAGE, process.memoryUsage().heapUsed];
...
function reportError(error, type) {
  ...
  _worker_threads().parentPort.postMessage([type, error.constructor && error.constructor.name, error.message, error.stack, _typeof(error) === 'object' ? _objectSpread({}, error) : error]);
}
