...
var getResolver = function getResolver(config) {
  var resolver = resolvers.get(config.id);

  if (!resolver) {
    throw new Error("Cannot find resolver for: ".concat(config.id));
  }
...
function setup(setupData) {
  ...
  try {
    for (var _iterator = setupData.serializableResolvers[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
      ...
      var moduleMap = _jestHasteMap()["default"].getStatic(config).getModuleMapFromJSON(serializableModuleMap);
    ...
    while (1) {
      switch (_context.prev = _context.next) {
        case 0:
          ...
          _context.prev = 1;
          _context.next = 4;
          return regeneratorRuntime.awrap((0, _runTest["default"])(path, globalConfig, config, getResolver(config), _objectSpread({}, context, {
