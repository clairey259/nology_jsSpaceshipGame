...
var _require3 = require('./range'),
    sliceRange = _require3.sliceRange;
...
var _require4 = require('@jridgewell/trace-mapping'),
    TraceMap = _require4.TraceMap;
...
function () {
  ...
  _createClass(V8ToIstanbul, [{
        ...
        while (1) {
          switch (_context.prev = _context.next) {
            ...
            case 5:
              ...
              _context.next = 13;
              return regeneratorRuntime.awrap(this.sourcesContentFromSources());

            case 13:
              this.sourceMap.sourcesContent = _context.sent;
    ...
    key: "sourcesContentFromSources",
    value: function sourcesContentFromSources() {
      var _this2 = this;

      var fileList;
      return regeneratorRuntime.async(function sourcesContentFromSources$(_context2) {
        while (1) {
          switch (_context2.prev = _context2.next) {
            case 0:
              fileList = this.sourceMap.sources.map(function (relativePath) {
                var realPath = _this2._resolveSource(_this2.rawSourceMap, relativePath);

                return readFile(realPath, 'utf-8').then(function (result) {
                  return result;
                })["catch"](function (err) {
                  debuglog("failed to load ".concat(realPath, ": ").concat(err.message));
                });
              });
              _context2.next = 3;
              return regeneratorRuntime.awrap(Promise.all(fileList));
    ...
    key: "destroy",
    value: function destroy() {// no longer necessary, but preserved for backwards compatibility.
    }
  }, {
    key: "_resolveSource",
    ...
    value: function applyCoverage(blocks) {
      ...
      blocks.forEach(function (block) {
        block.ranges.forEach(function (range, i) {
          var _this3$_maybeRemapSta = _this3._maybeRemapStartColEndCol(range),
              startCol = _this3$_maybeRemapSta.startCol,
              endCol = _this3$_maybeRemapSta.endCol,
              path = _this3$_maybeRemapSta.path,
              covSource = _this3$_maybeRemapSta.covSource;

          if (_this3.excludePath(path)) {
            return;
          }
          ...
          if (block.functionName === '(empty-report)') {
            ...
            lines = covSource.lines.filter(function (line) {
              line.count = 0;
              return true;
            });
            _this3.all = lines.length > 0;
          } else {
          ...
          if (block.isBlockCoverage) {
            _this3.branches[path] = _this3.branches[path] || []; // record branches.

            _this3.branches[path].push(new CovBranch(startLineInstance.line, startCol - startLineInstance.startCol, endLineInstance.line, endCol - endLineInstance.startCol, range.count)); // if block-level granularity is enabled, we still create a single
            // CovFunction tracking object for each set of ranges.


            if (block.functionName && i === 0) {
              _this3.functions[path] = _this3.functions[path] || [];

              _this3.functions[path].push(new CovFunction(block.functionName, startLineInstance.line, startCol - startLineInstance.startCol, endLineInstance.line, endCol - endLineInstance.startCol, range.count));
            }
          } else if (block.functionName) {
            _this3.functions[path] = _this3.functions[path] || []; // record functions.
    ...
    value: function toIstanbul() {
      ...
      return this.covSources.reduce(function (istanbulOuter, _ref2) {
        var source = _ref2.source,
            path = _ref2.path;
        return Object.assign(istanbulOuter, _this4.getInnerIstanbul(source, path));
      }, {});
    }
  }, {
    ...
    value: function _branchesToIstanbul(source, path) {
      ...
      this.branches[path].forEach(function (branch, index) {
        var srcLine = source.lines[branch.startLine - 1];
        var ignore = srcLine === undefined ? true : srcLine.ignore;
    ...
    value: function _functionsToIstanbul(source, path) {
      ...
      this.functions[path].forEach(function (fn, index) {
        var srcLine = source.lines[fn.startLine - 1];
        var ignore = srcLine === undefined ? true : srcLine.ignore;
