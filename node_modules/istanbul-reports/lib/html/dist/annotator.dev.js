...
var RE_LT = /</g;
var RE_GT = />/g;
var RE_AMP = /&/g; // eslint-disable-next-line
...
function annotateLines(fileCoverage, structuredText) {
  ...
  Object.entries(lineStats).forEach(function (_ref) {
...
function annotateStatements(fileCoverage, structuredText) {
  ...
  Object.entries(statementStats).forEach(function (_ref3) {
    ...
    var openSpan = lt + 'span class="' + (meta.skip ? 'cstat-skip' : 'cstat-no') + '"' + title('statement not covered') + gt;
...
function annotateFunctions(fileCoverage, structuredText) {
  ...
  Object.entries(fnStats).forEach(function (_ref5) {
    var _ref6 = _slicedToArray(_ref5, 2),
        fName = _ref6[0],
        count = _ref6[1];

    var meta = fnMeta[fName];
    var type = count > 0 ? 'yes' : 'no'; // Some versions of the instrumenter in the wild populate 'func'
    // but not 'decl':

    var decl = meta.decl || meta.loc;
    var startCol = decl.start.column;
    var endCol = decl.end.column + 1;
    var startLine = decl.start.line;
    var endLine = decl.end.line;
    var openSpan = lt + 'span class="' + (meta.skip ? 'fstat-skip' : 'fstat-no') + '"' + title('function not covered') + gt;
    var closeSpan = lt + '/span' + gt;
    var text;
...
function annotateBranches(fileCoverage, structuredText) {
  ...
  Object.entries(branchStats).forEach(function (_ref7) {
    var _ref8 = _slicedToArray(_ref7, 2),
        branchName = _ref8[0],
        branchArray = _ref8[1];

    var sumCount = branchArray.reduce(function (p, n) {
      return p + n;
    }, 0);
    ...
    var openSpan;
    var closeSpan;
    var text; // only highlight if partial branches are missing or if there is a
