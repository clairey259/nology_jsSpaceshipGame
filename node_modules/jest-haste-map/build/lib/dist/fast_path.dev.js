...
function relative(rootDir, filename) {
  return filename.indexOf(rootDir + path().sep) === 0 ? filename.substring(rootDir.length + 1) : path().relative(rootDir, filename);
}

var INDIRECTION_FRAGMENT = "..".concat(path().sep); // rootDir must be an absolute path and relativeFilename must be simple
