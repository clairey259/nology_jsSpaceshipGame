...
function (_EventEmitter) {
  ...
  _createClass(NodeWatcher, [{
    ...
    value: function normalizeChange(dir, event, file) {
      ...
      if (!file) {
        this.detectChangedFile(dir, event, function (actualFile) {
          if (actualFile) {
            _this3.processChange(dir, event, actualFile);
          }
        });
      } else {
    ...
    value: function processChange(dir, event, file) {
      ...
      fs.lstat(fullPath, function (error, stat) {
        if (error && error.code !== 'ENOENT') {
          ...
          if (event !== 'change') {
            _this4.watchdir(fullPath);

            if (common.isFileIncluded(_this4.globs, _this4.dot, _this4.doIgnore, relativePath)) {
              _this4.emitEvent(ADD_EVENT, relativePath, stat);
            }
          }
        } else {
          var registered = _this4.registered(fullPath);

          if (error && error.code === 'ENOENT') {
            _this4.unregister(fullPath);

            _this4.stopWatching(fullPath);

            _this4.unregisterDir(fullPath);

            if (registered) {
              _this4.emitEvent(DELETE_EVENT, relativePath);
            }
          } else if (registered) {
            _this4.emitEvent(CHANGE_EVENT, relativePath, stat);
          } else {
            if (_this4.register(fullPath)) {
              _this4.emitEvent(ADD_EVENT, relativePath, stat);
            }
          }
        }
    ...
    value: function emitEvent(type, file, stat) {
      ...
      var key = "".concat(type, "-").concat(file);
      var addKey = "".concat(ADD_EVENT, "-").concat(file);
      ...
      this.changeTimers[key] = setTimeout(function () {
        delete _this5.changeTimers[key];

        if (type === ADD_EVENT && stat.isDirectory()) {
          // Recursively emit add events and watch for sub-files/folders
          common.recReaddir(path.resolve(_this5.root, file), function emitAddDir(dir, stats) {
            this.watchdir(dir);
            this.rawEmitEvent(ADD_EVENT, path.relative(this.root, dir), stats);
          }.bind(_this5), function emitAddFile(file, stats) {
            this.register(file);
            this.rawEmitEvent(ADD_EVENT, path.relative(this.root, file), stats);
          }.bind(_this5), function endCallback() {}, _this5.checkedEmitError, _this5.ignored);
        } else {
          _this5.rawEmitEvent(type, file, stat);
        }
      }, DEFAULT_DELAY);
    }
    /**
