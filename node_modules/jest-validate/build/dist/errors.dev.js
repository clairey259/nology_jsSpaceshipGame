...
var errorMessage = function errorMessage(option, received, defaultValue, options, path) {
  ...
  var validTypes = Array.from(new Set(conditions.map(_jestGetType().getType)));
  var message = "  Option ".concat(_chalk()["default"].bold("\"".concat(path && path.length > 0 ? "".concat(path.join('.'), ".") : '').concat(option, "\"")), " must be of type:\n    ").concat(validTypes.map(function (e) {
    return _chalk()["default"].bold.green(e);
  }).join(' or '), "\n  but instead received:\n    ").concat(_chalk()["default"].bold.red((0, _jestGetType().getType)(received)), "\n\n  Example:\n").concat(formatExamples(option, conditions));
