...
var createCLIValidationError = function createCLIValidationError(unrecognizedOptions, allowedOptions) {
  ...
  if (unrecognizedOptions.length === 1) {
    ...
    message = "  Unrecognized option ".concat(_chalk()["default"].bold((0, _utils.format)(unrecognized)), ".").concat(didYouMeanMessage ? " ".concat(didYouMeanMessage) : '');
  } else {
    title += 's';
...
function validateCLIOptions(argv, options) {
  ...
  var unrecognizedOptions = Object.keys(argv).filter(function (arg) {
    return !allowedOptions.has((0, _camelcase()["default"])(arg, {
      locale: 'en-US'
    })) && !allowedOptions.has(arg) && (!rawArgv.length || rawArgv.includes(arg));
  }, []);
