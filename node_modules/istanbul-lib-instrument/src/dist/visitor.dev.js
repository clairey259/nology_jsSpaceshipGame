...
var _require3 = require('@istanbuljs/schema'),
    defaults = _require3.defaults;

var _require4 = require('./source-coverage'),
    SourceCoverage = _require4.SourceCoverage;

var _require5 = require('./constants'),
    SHA = _require5.SHA,
    MAGIC_KEY = _require5.MAGIC_KEY,
    MAGIC_VALUE = _require5.MAGIC_VALUE; // pattern for istanbul to ignore a section
...
function () {
  function VisitState(types, sourceFilePath, inputSourceMap) {
    ...
    var reportLogic = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : false;
  ...
  _createClass(VisitState, [{
    ...
    key: "increaseTrue",
    value: function increaseTrue(type, id, index, node) {
      var T = this.types;
      var tempName = "".concat(this.varName, "_temp");
    ...
    key: "validateTrueNonTrivial",
    value: function validateTrueNonTrivial(T, tempName) {
    ...
    key: "getBranchLogicIncrement",
    value: function getBranchLogicIncrement(path, branchName, loc) {
      var index = this.cov.addBranchPath(branchName, loc);
...
function coverLogicalExpression(path) {
  ...
  var b = this.cov.newBranch('binary-expr', path.node.loc, this.reportLogic);
  ...
  for (var i = 0; i < leaves.length; i += 1) {
    ...
    if (this.reportLogic) {
      var _increment = this.getBranchLogicIncrement(leaf, b, leaf.node.loc);

      if (!_increment[0]) {
        continue;
      }

      leaf.parent[leaf.property] = T.sequenceExpression([_increment[0], _increment[1]]);
      continue;
    }
...
function programVisitor(types) {
  ...
  var T = types;
  opts = _objectSpread({}, defaults.instrumentVisitor, {}, opts);
  var visitState = new VisitState(types, sourceFilePath, opts.inputSourceMap, opts.ignoreClassMethods, opts.reportLogic);
  return {
    ...
    exit: function exit(path) {
      ...
      var hash = createHash(SHA).update(JSON.stringify(coverageData)).digest('hex');
