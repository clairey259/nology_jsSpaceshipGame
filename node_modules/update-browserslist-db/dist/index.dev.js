...
var _require = require('./utils'),
    detectIndent = _require.detectIndent,
    detectEOL = _require.detectEOL;
...
function getBrowsers() {
  ...
  return browserslist().reduce(function (result, entry) {
    if (!result[entry[0]]) {
      result[entry[0]] = [];
    }
...
function diffBrowsers(old, current) {
  var browsers = Object.keys(old).concat(Object.keys(current).filter(function (browser) {
    return old[browser] === undefined;
  }));
  return browsers.map(function (browser) {
    var oldVersions = old[browser] || [];
    var currentVersions = current[browser] || [];
    var common = oldVersions.filter(function (v) {
      return currentVersions.includes(v);
    });
    var added = currentVersions.filter(function (v) {
      return !common.includes(v);
    });
    var removed = oldVersions.filter(function (v) {
      return !common.includes(v);
    });
    return removed.map(function (v) {
      return pico.red('- ' + browser + ' ' + v);
    }).concat(added.map(function (v) {
      return pico.green('+ ' + browser + ' ' + v);
    }));
...
module.exports = function updateDB() {
  var print = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : defaultPrint;
  ...
  if (listError) {
    ...
    var changes = diffBrowsers(oldList, newList);
