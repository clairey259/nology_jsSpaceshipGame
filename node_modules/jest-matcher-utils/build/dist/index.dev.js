...
var printDiffOrStringify = function printDiffOrStringify(expected, received, expectedLabel, receivedLabel, expand // CLI options: true if `--expand` or false if `--no-expand`
) {
  if (typeof expected === 'string' && typeof received === 'string' && expected.length !== 0 && received.length !== 0 && expected.length <= MAX_DIFF_STRING_LENGTH && received.length <= MAX_DIFF_STRING_LENGTH && expected !== received) {
    ...
    return "".concat(_expectedLine, "\n").concat(_receivedLine);
  }
  ...
  if (isLineDiffable(expected, received)) {
    ...
    if (typeof difference === 'string' && difference.includes("- ".concat(expectedLabel)) && difference.includes("+ ".concat(receivedLabel))) {
      return difference;
    }
  }
...
var pluralize = function pluralize(word, count) {
  return "".concat(NUMBERS[count] || count, " ").concat(word).concat(count === 1 ? '' : 's');
