...
function parse(g) {
  ...
  if (!A) throw new Error("Not initialized");
  var D = g.length + 1,
      N = (A.__heap_base.value || A.__heap_base) + 4 * D - A.memory.buffer.byteLength;
  N > 0 && A.memory.grow(Math.ceil(N / 65536));
  var k = A.sa(D);
  if ((Q ? C : E)(g, new Uint16Array(A.memory.buffer, k, D)), !A.parseCJS(k, g.length, 0, 0, 0)) throw Object.assign(new Error("Parse error ".concat(I).concat(A.e(), ":").concat(g.slice(0, A.e()).split("\n").length, ":").concat(A.e() - g.lastIndexOf("\n", A.e() - 1))), {
    idx: A.e()
  });
  var w = new Set(),
      o = new Set(),
  ...
  for (; A.rre();) {
    var _Q = B(g.slice(A.res(), A.ree()));

    _Q && o.add(_Q);
  }

  for (; A.ru();) {
    J.add(B(g.slice(A.us(), A.ue())));
  }

  for (; A.re();) {
    var _Q2 = B(g.slice(A.es(), A.ee()));

    void 0 === _Q2 || J.has(_Q2) || w.add(_Q2);
  }

  return {
    exports: _toConsumableArray(w),
    reexports: _toConsumableArray(o)
  };
}
...
function B(A) {
  ...
  try {
    ...
    for (var _A = 0; _A < _Q3.length; _A++) {
      var _B = 64512 & _Q3.charCodeAt(_A);

      if (!(_B < 55296)) {
        if (55296 !== _B) return;
        if (56320 != (64512 & _Q3.charCodeAt(++_A))) return;
      }
    }
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          ...
          case 2:
            Q = _context.sent;
            _context.next = 5;
            return regeneratorRuntime.awrap(WebAssembly.instantiate(Q));
