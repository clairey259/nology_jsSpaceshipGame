...
var jestReadFile = globalThis[_Symbol["for"]('jest-native-read-file')] || fs.readFileSync; // stack utils tries to create pretty stack by making paths relative.
...
var indentAllLines = function indentAllLines(lines) {
  return lines.replace(NOT_EMPTY_LINE_REGEXP, MESSAGE_INDENT);
};
...
function warnAboutWrongTestEnvironment(error, env) {
  return _chalk["default"].bold.red("The error below may be caused by using the wrong test environment, see ".concat(_chalk["default"].dim.underline('https://jestjs.io/docs/configuration#testenvironment-string'), ".\nConsider using the \"").concat(env, "\" test environment.\n\n")) + error;
...
var formatExecError = function formatExecError(error, config, options, testPath, reuseMessage) {
  ...
  if (typeof error === 'string' || !error) {
    ...
    stack = typeof error.stack === 'string' ? error.stack : "thrown: ".concat((0, _prettyFormat.format)(error, {
      maxDepth: 3
    }));
  }
  ...
  return "".concat(TITLE_INDENT + TITLE_BULLET + messageToUse + stack, "\n");
};
...
var formatPath = function formatPath(line, config) {
  var relativeTestPath = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : null;
...
var formatStackTrace = function formatStackTrace(stack, config, options, testPath) {
  ...
  var stacktrace = lines.filter(Boolean).map(function (line) {
    return STACK_INDENT + formatPath(trimPaths(line), config, relativeTestPath);
  }).join('\n');
...
var formatResultsErrors = function formatResultsErrors(testResults, config, options, testPath) {
  var failedResults = testResults.reduce(function (errors, result) {
    result.failureMessages.forEach(function (item) {
      errors.push({
  ...
  return failedResults.map(function (_ref) {
    ...
    stack = options.noStackTrace ? '' : "".concat(STACK_TRACE_COLOR(formatStackTrace(stack, config, options, testPath)), "\n");
    message = indentAllLines(message);
    var title = "".concat(_chalk["default"].bold.red(TITLE_INDENT + TITLE_BULLET + result.ancestorTitles.join(ANCESTRY_SEPARATOR) + (result.ancestorTitles.length ? ANCESTRY_SEPARATOR : '') + result.title), "\n");
    return "".concat(title, "\n").concat(message, "\n").concat(stack);
  }).join('\n');
};
