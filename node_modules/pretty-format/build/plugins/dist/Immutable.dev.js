...
var printImmutableEntries = function printImmutableEntries(val, config, indentation, depth, refs, printer, type) {
  return ++depth > config.maxDepth ? printAsLeaf(getImmutableName(type)) : "".concat(getImmutableName(type) + SPACE, "{").concat((0, _collections.printIteratorEntries)(val.entries(), config, indentation, depth, refs, printer), "}");
...
var printImmutableRecord = function printImmutableRecord(val, config, indentation, depth, refs, printer) {
  ...
  return ++depth > config.maxDepth ? printAsLeaf(name) : "".concat(name + SPACE, "{").concat((0, _collections.printIteratorEntries)(getRecordEntries(val), config, indentation, depth, refs, printer), "}");
};
...
var printImmutableSeq = function printImmutableSeq(val, config, indentation, depth, refs, printer) {
  ...
  if (val[IS_KEYED_SENTINEL]) {
    return "".concat(name + SPACE, "{").concat( // from Immutable collection of entries or from ECMAScript object
    val._iter || val._object ? (0, _collections.printIteratorEntries)(val.entries(), config, indentation, depth, refs, printer) : LAZY, "}");
  }

  return "".concat(name + SPACE, "[").concat(val._iter || // from Immutable collection of values
...
var printImmutableValues = function printImmutableValues(val, config, indentation, depth, refs, printer, type) {
  return ++depth > config.maxDepth ? printAsLeaf(getImmutableName(type)) : "".concat(getImmutableName(type) + SPACE, "[").concat((0, _collections.printIteratorValues)(val.values(), config, indentation, depth, refs, printer), "]");
};
